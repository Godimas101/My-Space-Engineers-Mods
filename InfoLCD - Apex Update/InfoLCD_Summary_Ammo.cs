...
namespace SG.LCDInfo
{
    ...
    public class LCDAmmoSummaryInfo : MyTextSurfaceScriptBase
    {
        ...
        void DrawMainSprite(ref MySpriteDrawFrame frame, ref Vector2 position)
        {
            try
            {
                ...
                if (surfaceData.showSummary)
                {
                    ...
                    volume /= 1000;
                    SurfaceDrawer.DrawBar(ref frame, ref position, surfaceData, "Ammo", volume, current, Unit.Percent, false, true);
                    position += surfaceData.newLine;
                }
        ...
        void DrawAllKnownSprite(ref MySpriteDrawFrame frame, ref Vector2 position)
        {
            try
            {
                SurfaceDrawer.WriteTextSprite(ref frame, position, surfaceData, $"Id [Ammo]", TextAlignment.LEFT, surfaceData.surface.ScriptForegroundColor);
