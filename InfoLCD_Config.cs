...
namespace SG.LCDInfo
{
    ...
    public static class SGUtillities
    {
        ...
        public static float GetPowerTimeLeft(List<IMyPowerProducer> powerProducers)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (reactors.Count > 0)
                    {
                        ...
                        {
                            timeRemaining = (SGUtillities.GetItemAmountFromBlockList(reactors.Select(x => x as IMyCubeBlock).ToList(), "Ingot", "Uranium") / currentReactorOutput) * 3600;
                        }
                    }
                }
            }
            ...
            {
                MyLog.Default.WriteLine($"SG.LCDInfo.LCDInfoScreenPowerSummary: Caught Exception while calculating PowerTimeLeft: {e.ToString()}");
            }
        ...
        public static float GetItemAmountFromBlockList(List<IMyCubeBlock> blocks, string targetTypeId, string targetSubtypeId)
        {
            ...
            {
                MyLog.Default.WriteLine($"SG.LCDInfo.LCDInfoScreenPowerSummary: Caught Exception while GetItemAmountFromBlockList: {e.ToString()}");
            }
        ...
        public static GasTankVolumes GetGasTankVolumeFromTanks (List<IMyGasTank> allTanks, string gasSubtypeId)
        {
            ...
            {
                MyLog.Default.WriteLine($"SG.LCDInfo.LCDInfoScreenPowerSummary: Caught Exception while GetGasTankVolumeFromTanks: {e.ToString()}");
            }
        ...
        public static List<MyCubeBlock> GetBlocks (MyCubeGrid cubeGrid, string searchId, List<string> excludeIds, ref Sandbox.ModAPI.Ingame.MyShipMass gridMass, bool includeSubGrids = false, bool includeDocked = false)
        {
            ...
            {
                MyLog.Default.WriteLine($"SG.LCDInfo.SGUtillities: Caught Exception while GetBlocks: {e.ToString()}");
            }
    ...
    public struct BlockStateData
    {
        ...
        {
            this.block = block;
            slimBlock = SGUtillities.GetSlimblock(block);
